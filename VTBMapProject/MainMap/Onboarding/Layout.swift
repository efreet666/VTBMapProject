...
protocol FlowLayoutDelegate: AnyObject {
	...
	open override func layoutAttributesForElements(in rect: CGRect) -> [UICollectionViewLayoutAttributes]? {
		...
		if scrollDirection == .vertical {
			for layoutAttributes in layoutAttributesForElements {
				...
				guard let currentItemAttributes = layoutAttributes.copy() as? UICollectionViewLayoutAttributes else { return [UICollectionViewLayoutAttributes()] }
				// if the current frame, once stretched to the full row doesn't intersect the previous frame then they are on different rows
				if previousFrame != nil && !currentItemAttributes.frame.intersects(CGRect(x: -.greatestFiniteMagnitude,
			...
			return representedElements + cells.flatMap { group -> [UICollectionViewLayoutAttributes] in
				...
				var origin: CGFloat = (collectionView.bounds.width + evaluatedSectionInset.left - evaluatedSectionInset.right - group.reduce(0, { $0 + $1.frame.size.width })
									   - CGFloat(group.count - 1) * evaluatedMinimumInteritemSpacing) / 2
			...
			for layoutAttributes in layoutAttributesForElements {
				...
				if previousFrame != nil && !currentItemAttributes.frame.intersects(CGRect(x: previousFrame!.origin.x,
			...
			return representedElements + cells.flatMap { group -> [UICollectionViewLayoutAttributes] in
				...
				let evaluatedSectionInset = evaluatedSectionInsetForSection(at: section)
				let evaluatedMinimumInteritemSpacing = evaluatedMinimumInteritemSpacingForSection(at: section)
				var origin: CGFloat = (collectionView.bounds.height + evaluatedSectionInset.top - evaluatedSectionInset.bottom - group.reduce(0, { $0 + $1.frame.size.height })
									   - CGFloat(group.count - 1) * evaluatedMinimumInteritemSpacing) / 2
				// we reposition each element of a group
				return group.map {
